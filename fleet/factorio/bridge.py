...
class FactorioBridge:
    ...
    def __init__(self, config: BridgeConfig):
        ...
        # Adaptive tick governor — paces RL actions to Factorio UPS
        from factorio.tick_governor import TickGovernor
        target_ups = 60 * max(config.game_speed, 1)  # nominal ticks/sec
        self._tick_governor = TickGovernor(
            delay_min_ms=config.ml_tick_delay_min_ms,
            delay_max_ms=config.ml_tick_delay_max_ms,
            target_ups=target_ups,
            teacher_delay_ms=config.ml_teacher_delay_ms,
            window_size=config.ml_ups_window_size,
        )
        self._teacher_cooldown_max = config.ml_teacher_cooldown_ticks
    ...
    async def _ml_tick_agent(self, agent_id: int = 1) -> None:
        ...
        # Feed tick governor with UPS sample — agent 1 only (all agents share game tick).
        # With num_agents > 1, recording per-agent would corrupt UPS measurement.
        if agent_id == 1:
            import time as _time
            self._tick_governor.record_tick(state.tick, _time.monotonic())
        ...
        # ── Teacher-exclusive mode ─────────────────────────────────
        # When teacher has queued actions OR is in cooldown, RL is paused.
        # The LLM has "admin" control — success > speed.
        if self._teacher_actions:
            self._tick_governor.set_teacher_mode(True)
            ...
            # Reset cooldown after each teacher action
            self._teacher_cooldown = self._teacher_cooldown_max
            # RL does NOT act this tick
        ...
        if self._teacher_cooldown > 0:
            ...
            if self._teacher_cooldown == 0:
                self._tick_governor.set_teacher_mode(False)
                log.info("Teacher cooldown complete — RL resuming")
            # RL paused during cooldown too
            self._prev_state = state
            return

        # If we reach here, teacher is idle — ensure governor is in normal mode
        self._tick_governor.set_teacher_mode(False)

        # Launch teacher in background if stuck (non-blocking)
        if (self._teacher_lesson_step_count >= self._teacher_stuck_threshold
    ...
    async def run(self) -> None:
        ...
        while self._running:
            if self.config.mode == "ml":
                await self.ml_tick()
                # Adaptive tick delay — governor scales with Factorio UPS
                # Override: ml_tick_delay_ms > 0 forces fixed delay (escape hatch)
                if self.config.ml_tick_delay_ms > 0:
                    await asyncio.sleep(self.config.ml_tick_delay_ms / 1000.0)
                else:
                    delay_ms = self._tick_governor.get_delay_ms()
                    await asyncio.sleep(delay_ms / 1000.0)
                # Log governor stats every 100 ticks
                if self._tick_count % 100 == 0 and self.config.ml_tick_delay_ms == 0:
                    log.info("Tick governor: UPS=%.1f delay=%dms teacher=%s",
                             self._tick_governor.observed_ups,
                             self._tick_governor.get_delay_ms(),
                             self._tick_governor.is_teacher_mode)
            else:
                await self.tick()
